...
def _get_mediapipe_solutions():
    global _mp_pose, _mp_hands, _mediapipe_available
    if _mediapipe_available is None:
        try:
            ...
            if hasattr(mp, 'solutions'):
                ...
                logger.info("MediaPipe Pose/Hands loaded successfully (solutions API)")
            else:
                _mediapipe_available = False
                logger.warning("MediaPipe 'solutions' API not available. Liveness checks disabled.")
        except ImportError:
            _mediapipe_available = False
            logger.warning("MediaPipe not installed. Liveness checks disabled.")
        except Exception as e:
            _mediapipe_available = False
            logger.warning(f"MediaPipe initialization failed: {e}. Liveness checks disabled.")
    return _mp_pose, _mp_hands
...
class LivenessChallengeService:
    ...
    def __init__(self):
        mp_pose, mp_hands = _get_mediapipe_solutions()
        self.mp_pose = mp_pose
        self.mp_hands = mp_hands
        self.pose = None
        
        # Only initialize pose if mediapipe is available
    ...
    def verify_challenge(self, image_bytes: bytes, challenge_type: str) -> bool:
        # Return False if MediaPipe is not available
        if self.pose is None:
            logger.warning("Liveness check skipped: MediaPipe not available")
            return True  # Skip check, assume passing to avoid blocking workflow
