...
from app.agents.interview_agent.selenium.meet_controller import MeetController
...
class MeetSessionManager:
    ...
    async def create_interview_session(
        ...
        """Creates a session record in memory and optionally generates a Meet link. Async."""
    ...
    def start_bot_session(
        ...
        WARNING: This is synchronous and blocking (Selenium). Run in a thread.
        """
        try:
            ...
            # NOTE: Video capture is NOT started here - it's started in background task
    ...
    async def get_snapshot_count(self, session_id: str) -> int:
        """Async version of snapshot count retrieval."""
        session = self.active_sessions.get(session_id)
        if session:
