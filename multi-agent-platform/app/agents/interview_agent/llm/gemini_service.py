...
# app/services/interview/llm/gemini_service.py
import os
import re
import json
import logging
...
from app.agents.interview_agent.utils.retry_handler import retry_on_failure
...
class GeminiService:
    ...
    def _initialize_client(self):
        ...
        try:
            # Use centralized secrets manager (Note: Adjust import as needed for HR-Automation env)
            # Assuming app.config.secrets exists or we use os.environ
    ...
    def _log_token_usage(self, session_id, last_chunk):
        try:
            ...
            if usage:
                ...
                # Check for running loop to handle async DB call
                try:
                    loop = asyncio.get_running_loop()
                    loop.create_task(self.db.update_gemini_token_usage(session_id, "interview", p, r, t))
                except RuntimeError:
                    # Sync context fall back (e.g. if called from thread without loop accessible easily)
                     logger.warning("Could not log token usage: no event loop")
        except Exception as e: 
