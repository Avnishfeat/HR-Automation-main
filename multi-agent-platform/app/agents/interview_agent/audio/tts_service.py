...
# app/services/interview/audio/tts_service.py
import os
import logging
...
class TTSService:
    ...
    def __init__(self, db_handler: SessionRepository):
        ...
        self.loop = None # For thread-safe DB calls
    ...
    async def get_or_create_static_audio(self, text: str, filename_key: str) -> Optional[Path]:
        """
        ...
        # DB calls are async
        try:
            db_audio = await self.db.get_file(filename_key)
        except Exception as e:
            logger.error(f"DB get_file failed: {e}")
            db_audio = None
        
        if db_audio:
            logger.info(f"Found in DB, downloading to local cache: {file_path}")
            # File IO can be blocking, but usually fast. 
        ...
        try:
            ...
            # A. Save to MongoDB (Golden Source) - Async
            await self.db.save_file(filename=filename_key, data=audio_content)
    ...
    def _log_usage_and_save(self, text: str, session_id: str, turn_count: Any, candidate_id: str, is_follow_up: bool):
        ...
        if self.loop:
            try:
                ...
                logger.warning(f"Failed to log TTS usage async: {e}")
        else:
             logger.warning("No loop available for TTS usage logging")
